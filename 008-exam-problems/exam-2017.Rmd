...
```
```r
> drugmod1 <- lm(Effectiveness ~ Drug + Disease + r(Hospital),  data=drugdata) 
> summary(drugmod1) 
```
```
Call: 
lm(formula = Effectiveness ~ Drug + Disease + r(Hospital),  data = drugdata) 
...
Call:lm(formula = Effectiveness ~ Drug + Disease + Drug:Disease +  r(Hospital), data = drug) 
